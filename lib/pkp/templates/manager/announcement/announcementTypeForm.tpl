...
<form id="announcementTypeForm" method="post" action="{url op="updateAnnouncementType"}" class="form-horizontal">
{if $typeId}
...
<div class="form-group">
<div class="col-md-3 col-sm-3 control-label">
{fieldLabel name="name" required="true" key="manager.announcementTypes.form.typeName"}
</div>
<div class="col-md-7 col-sm-7">
...
<p><input type="submit" value="{translate key="common.save"}" class="btn btn-primary" /> {if not $typeId}<input type="submit" name="createAnother" value="{translate key="manager.announcementTypes.form.saveAndCreateAnother"}" class="btn btn-primary" /> {/if}<input type="button" value="{translate key="common.cancel"}" class="btn btn-warning" onclick="document.location.href='{url op="announcementTypes" escape=false}'" /></p>
</div>
</div>
</form>
</div>
