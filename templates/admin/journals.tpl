...
<div id="journals">
<a href="{url op="createJournal"}" class="btn btn-success pull-right">{translate key="admin.journals.create"}</a>
<table width="100%" class="table table-striped" id="adminJournals">
	...
	<tr valign="top" id="journal-{$journal->getId()}" class="data">
		...
		<td align="right"><a href="{url op="editJournal" path=$journal->getId()}" class="btn btn-warning btn-xs">{translate key="common.edit"}</a>&nbsp;|&nbsp;<a class="btn btn-danger btn-xs" href="{url op="deleteJournal" path=$journal->getId()}" onclick="return confirm('{translate|escape:"jsparam" key="admin.journals.confirmDelete"}')">{translate key="common.delete"}</a></td>
	</tr>
	{/iterate}
